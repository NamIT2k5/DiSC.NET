...
namespace BasicNet
{
    ...
    public class MinimizerBarnesHut
    {
        ...
        private Dictionary<Node, ICollection<Interaction>> attrEdges;
        ...
        private double[] baryCenter = new double[3];
        ...
        private const int nrDims = 3;
        /** Position in 3-dimensional space for each node. */
        private Dictionary<Node, double[]> positions;
        ...
        public MinimizerBarnesHut(
                ICollection<Node> nodes, ICollection<Interaction> attrEdges,
                double repuExponent, double attrExponent, double gravFactor)
        {
            this.nodes = new List<Node>(nodes);
            this.attrEdges = new Dictionary<Node, ICollection<Interaction>>();
            foreach (Node node in nodes) this.attrEdges[node] = new List<Interaction>();
            foreach (Interaction edge in attrEdges)
            {
                if (edge.startNode == edge.endNode) continue;
                this.attrEdges[edge.startNode].Add(edge);
            }
         ...
         */
        public void minimizeEnergy(Dictionary<Node, double[]> positions, int nrIterations)
        {
            ...
            foreach (Node node in nodes) energySum += getEnergy(node, octTree);
            //System.out.println("initial energy " + energySum);
            Debug.WriteLine("initial energy " + energySum);


            // minimize energy
            double[] oldPos = new double[nrDims];
            double[] bestDir = new double[nrDims];
            for (int step = 1; step <= nrIterations; step++)
            {
                ...
                Debug.WriteLine("iteration " + step
                  + "   energy " + energySum
                  + "   repulsion " + repuExponent);
            }
            printStatistics(octTree);
        ...
        private void initEnergyFactors()
        {
            ...
            foreach (Node node in nodes)
            {
                foreach (Interaction edge in attrEdges[node])
                {
                    attrSum += edge.weight;
                }
            }
            double repuSum = 0.0;
            foreach (Node node in nodes) repuSum += node.weight;
         ...
         */
        private double getDist(double[] pos1, double[] pos2)
        {
            double dist = 0.0;
         ...
         */
        private double getRepulsionEnergy(Node node, OctTree tree)
        {
            if (tree == null || tree.node == node) return 0.0;
            if (node.weight == 0.0) return 0.0;

            double dist = getDist(positions[node], tree.position);
        ...
        {
            double energy = 0.0;
            double[] position = positions[node];
            foreach (Interaction edge in attrEdges[node])
            {
                double dist = getDist(position, positions[edge.endNode]);
                if (attrExponent == 0.0)
                {
        ...
        {
            double dist = getDist(positions[node], baryCenter);
            if (attrExponent == 0.0)
            {
         ...
         */
        private double getEnergy(Node node, OctTree octTree)
        {
         ...
         */
        private double addRepulsionDir(Node node, OctTree tree, double[] dir)
        {
            if (tree == null || tree.node == node) return 0.0;
            if (node.weight == 0.0) return 0.0;
         ...
         */
        private double addAttractionDir(Node node, double[] dir)
        {
            double dir2 = 0.0;
            double[] position = positions[node];
            foreach (Interaction edge in attrEdges[node])
            {
                double[] position2 = positions[edge.endNode];
         ...
         */
        private double addGravitationDir(Node node, double[] dir)
        {
            double[] position = positions[node];
         ...
         *               (output parameter)
         */
        private void getDirection(Node node, OctTree octTree, double[] dir)
        {
        ...
        private OctTree buildOctTree()
        {
            ...
            double[] minPos = { Double.MaxValue, Double.MaxValue, Double.MaxValue };
            double[] maxPos = { -Double.MaxValue, -Double.MaxValue, -Double.MaxValue };
            foreach (Node node in nodes)
            {
            ...
            foreach (Node node in nodes) result.addNode(node, positions[node], 0);
            return result;
        }
        ...
        private void printStatistics(OctTree octTree)
        {
            ...
            Debug.WriteLine("Number of nodes: " + nodes.Count);
            double attrSum = 0.0;
            foreach (Node node in nodes)
            {
                foreach (Interaction edge in attrEdges[node])
            ...
            Debug.WriteLine("Overall attraction: " + attrSum);

            double meanAttrEnergy = 0.0;
            foreach (Node node in nodes) meanAttrEnergy += getAttractionEnergy(node);
            ...
            Debug.WriteLine("Weighted mean of attraction energy: " + meanAttrEnergy);
            ...
            Debug.WriteLine("Overall repulsion: " + repuSum);
            double meanRepuEnergy = 0.0;
            foreach (Node node in nodes) meanRepuEnergy += getRepulsionEnergy(node, octTree);
            ...
            Debug.WriteLine("Weighted mean of repulsion energy: " + meanRepuEnergy);

            //System.out.println("Mean attraction / mean repulsion: " + meanAttrEnergy / meanRepuEnergy);
            Debug.WriteLine("Mean attraction / mean repulsion: " + meanAttrEnergy / meanRepuEnergy);
        }
        ...
        protected class OctTree
        {
            ...
            public void removeNode(Node oldNode, double[] oldPos, int depth)
            {
                ...
                else
                {
                    ...
                    if (children[childIndex] == null) return; //Mr. Dzung add this line to avoid an error
