...
namespace MathNet.Numerics.LinearAlgebra
{
    ...
    public class EigenvalueDecomposition
    {
        ...
        /// <summary>
        /// Programmed by: Tran Tien Dzung (Dec, 05, 2012); see the algorithm at http://en.wikipedia.org/wiki/Power_iteration
        /// produce a number λ (the eigenvalue) and a nonzero vector v (the eigenvector), such that Av = λv. The algorithm is also known as the Von Mises iteration
        ///  It does not compute a matrix decomposition. However, it will find only one eigenvalue (the one with the greatest absolute value) and it may converge only slowly
        /// </summary>
        /// <param name="A">The matrix to compute an eigenvalue and an eigenvector that both are dominant</param>
        /// <returns>A pair of (eigenvector, eigenvalue)</returns>
        public static Vector DominantEigenvector(Matrix A, out double dominantEigenValue, double epsilon=0.00001)
        {
            Vector tmp = new Vector(A.RowCount, 0.0), b = new Vector(A.RowCount, 1.0);
            int n = A.RowCount;
            double oldnorm = 0.0;
            double norm = 100;
            ...
            while (true)
            {
                ...
                if (Math.Abs(norm - oldnorm) <= epsilon) break;
