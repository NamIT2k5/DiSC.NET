...
namespace MatrixLibrary
{
	...
	public class Matrix
	{
        ...
        public static double SumCells(double[,] Mat)
        {
            int i, j;
            int nRow, nCol;
            ...
            try
            {
                Find_R_C(Mat, out nRow, out nCol);
            }
            catch
            {
        ...
        public double SumCell()
        {
            return Matrix.SumCells(this.in_Mat);
        }
        #region Array formula

        public delegate double OneOperandAction(double num);
        public static double[,] ArrayFormula(double[,] Mat, OneOperandAction function)
        {
            double[,] sol;
            int i, j;
            int nRow, nCol;
            ...
            sol = new double[nRow + 1, nCol + 1];

            for (i = 0; i <= nRow; i++)
                for (j = 0; j <= nCol; j++)
                {
                    sol[i, j] = function(Mat[i, j]);
                }

            return sol;
        }
        /// <summary>
        /// Operating on each matrix's element
        /// </summary>
        /// <param name="function">The function handling on each matrix's element</param>
        /// <returns>the result</returns>
        public Matrix ArrayFormula(OneOperandAction function)
        {
            return new Matrix(Matrix.ArrayFormula(this.in_Mat, function));
        }
        public delegate double TwoOperandAction(double num1, double num2);

        public static double[,] ArrayFormula(double[,] Mat1, double[,] Mat2, TwoOperandAction function)
        {
            double[,] sol;
            int i, j;
            int Rows1, Cols1;
            ...
            for (i = 0; i <= Rows1; i++)
                for (j = 0; j <= Cols1; j++)
                {
                    sol[i, j] = function(Mat1[i, j], Mat2[i, j]);
                }

            return sol;
        }
        public Matrix ArrayFormula(Matrix Ma, TwoOperandAction function)
        {
            return new Matrix(Matrix.ArrayFormula(this.in_Mat, Ma.in_Mat, function));
        }
        #endregion
